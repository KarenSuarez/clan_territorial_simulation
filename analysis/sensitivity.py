# clan_territorial_simulation/analysis/sensitivity.py
import numpy as np
...
def run_monte_carlo_simulation(num_runs=100, num_steps=100, dt=0.1, seed=None):
    ...
    for i in range(num_runs):
        ...
        initial_clans = []
        for j in range(rng.randint(1, INITIAL_CLAN_COUNT + 1)): # Número variable de clanes
            ...
            initial_clans.append(Clan(j + 1, initial_size, initial_position,
                                       parameters={'birth_rate': rng.uniform(0.05, 0.2),
                                                   'natural_death_rate': rng.uniform(0.01, 0.1)}))

        environment = Environment(grid_size=GRID_SIZE)
        engine = SimulationEngine(environment, initial_clans, simulation_params={'dt': dt}, seed=rng.randint(0, 1000000))
...
def analyze_sensitivity_results(monte_carlo_results):
    ...
    Calcula correlaciones y busca configuraciones críticas.
    """
    ...
    for result in monte_carlo_results:
        ...
        initial_params.append({
            ...
            # Añadir más parámetros iniciales relevantes
        })
        final_populations.append(sum(c['size'] for c in final_state['clans']))

    # Calcular correlaciones (ejemplo con la población final)
    correlations = {}
    if initial_params and final_populations:
        ...
        correlations['num_clans_vs_final_population'] = num_clans_corr

        avg_size_corr = np.corrcoef([p['avg_initial_size'] for p in initial_params], final_populations)[0][1]
        correlations['avg_initial_size_vs_final_population'] = avg_size_corr

        # Calcular correlaciones para tasas de nacimiento y muerte (promedios)
        ...
        correlations['avg_birth_rate_vs_final_population'] = birth_rate_corr
        ...
        correlations['avg_death_rate_vs_final_population'] = death_rate_corr

    # Identificar configuraciones críticas (ejemplo: aquellas con extinción total)
    critical_configs = [res['initial_conditions'] for res in monte_carlo_results if sum(c['size'] for c in res['final_state']['clans']) == 0]
...
def generate_sensitivity_report(analysis_results):
    ...
    if analysis_results.get('critical_configurations'):
        if analysis_results['critical_configurations']:
            ...
            for i, config in enumerate(analysis_results['critical_configurations'][:5]): # Mostrar las primeras 5
                report += f"  - Configuración {i+1}: {config}\n"
